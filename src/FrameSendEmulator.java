...
public class FrameSendEmulator {
    private SerialPort serialPort;
    private Random random = new Random();

    public FrameSendEmulator(String portName, int baudRate) {
    ...
    public void startSendingData() {
        Thread sendingThread = new Thread(() -> {
            while (true) {
                ...
                byte[] gpggaByte = gpgga.getBytes();
                for(byte b : gpggaByte){
                    serialPort.writeBytes(new byte[]{b}, 1);
                    System.out.print((char) b);
                }
                
                byte[] gprmcByte = gprmc.getBytes();
                for(byte b : gprmcByte){
                    serialPort.writeBytes(new byte[]{b}, 1);
                    try {
                        Thread.sleep(5);
    ...
    public static void main(String[] args) {
        ...
        FrameSendEmulator emulator = new FrameSendEmulator(portName, baudRate);
